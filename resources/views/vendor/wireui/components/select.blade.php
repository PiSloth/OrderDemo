<div {{ $attributes->only(['class', 'wire:key'])->class('relative w-full') }} x-data="wireui_select({
        ...
        <x-dynamic-component :component="WireUi::component('input')"
            ...
            <x-slot name="prepend">
                <div
                    ...
                    <template x-if="config.multiselect">
                        <div class="absolute left-0 inset-y-0 pl-3 pr-14 w-full flex items-center overflow-hidden"
                            x-on:click="toggle">
    ...
    <x-wireui::parts.popover :margin="(bool) $label" class="sm:max-w-xs">
        <template x-if="asyncData.api || (config.searchable && options.length >= @toJs($minItemsForSearch))">
            <div class="px-2 my-2" wire:model.live='xSelectInput' wire:key="search.options.{{ $name }}">
        ...
        <div class="max-h-64 sm:max-h-60 overflow-y-auto overscroll-contain soft-scrollbar select-none" tabindex="-1"
