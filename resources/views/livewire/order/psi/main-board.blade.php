<div>
        ...
        <div class="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-lg overflow-hidden">
            <div class="p-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <div>
                    ...
                    <div class="text-sm text-gray-500 dark:text-gray-300">Last {{ (int) ($monthly_months ?? 6) }}
                        months by product</div>
                </div>
            ...
            <div class="px-4 pb-4">
                <details class="rounded-md border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/50">
                    ...
                    <div class="px-4 pb-4 text-sm text-gray-700 dark:text-gray-200 space-y-2">
                        <div>
                            <span class="font-semibold">Branch Filter</span> — “All branches” ဆိုရင် ဆိုင်ခွဲအားလုံးရဲ့
                            sale ကိုပေါင်းပြမယ်။ ဆိုင်ခွဲတစ်ခုရွေးရင် အဲဒီ branch ရဲ့ sale ပဲပြမယ်။
                        </div>
                        <div>
                            <span class="font-semibold">Metric</span> — Quantity / Total grams / Index ကိုရွေးပြီး
                            ဇယားထဲက amount တွေကို အဲဒီ metric အတိုင်းတွက်ပြပါတယ်။
                        </div>
                        <div>
                            <span class="font-semibold">Months Filter</span> — နောက်ဆုံး ဘယ်နှစ်လ (3/6/9/12) ကိုပြမလဲ
                            ရွေးနိုင်ပါတယ်။
                        </div>
                        <div>
                            <span class="font-semibold">Column (Months)</span> — လက်ရှိလမှ နောက်ဆုံး <span
                                class="font-semibold">{{ (int) ($monthly_months ?? 6) }} လ</span> အတွင်း Product
                            တစ်ခုချင်းစီရဲ့ Monthly Actual Sale (qty) ကိုပြထားပါတယ်။
                        </div>
                        <div>
                            <span class="font-semibold">အရောင်အလင်းပြ (Highlight)</span> — အစိမ်းနု cell က အဲဒီ Product
                            အတွက် “အမြင့်ဆုံး (Highest)” လ ဖြစ်ပါတယ်။ အနီနု cell က “အနိမ့်ဆုံး (Lowest)” လ ဖြစ်ပါတယ်။
                        </div>
                        <div>
                            <span class="font-semibold">Highest / Lowest</span> — ဘယ်လမှာ အများဆုံး/အနည်းဆုံးဖြစ်လဲ +
                            amount ကို အကွက်အခြားအဖြစ် ပြထားပါတယ်။
                        </div>
                        <div>
                            <span class="font-semibold">Focus</span> — Product ရဲ့ Focus Qty (သတ်မှတ်ထားတဲ့ focus) ကို
                            Metric အလိုက်တွက်ပြထားပါတယ်။
                        </div>
                        <div>
                            <span class="font-semibold">Current (Situation)</span> — လက်ရှိလ sale qty ကိုပြပြီး၊
                            အရင်လနဲ့ယှဉ်ပြီး <span class="font-semibold">Up / Down / Flat</span> + % ပြထားပါတယ်။
                        </div>
                        <div>
                            <span class="font-semibold">Current vs Focus</span> — လက်ရှိလ Actual − Focus ကိုပြပြီး <span
                                class="font-semibold">Above / Below / On focus</span> ဆိုပြီး status ပြထားပါတယ်။
                        </div>
                    </div>
                </details>
            </div>
            ...
            <div class="overflow-auto max-h-[60vh]">
                @php
                    ...
                    foreach ($monthKeys as $k) {
                        ...
                        if ($totalMaxAmount === null || $amt > $totalMaxAmount) {
                            $totalMaxAmount = $amt;
                            $totalMaxKey = $k;
                        }
                        if ($totalMinAmount === null || $amt < $totalMinAmount) {
                            $totalMinAmount = $amt;
                            $totalMinKey = $k;
                        }
                    }
                    ...
                    $totalDeltaPct =
                        $totalPrev > 0 ? ($totalDelta / $totalPrev) * 100.0 : ($totalCurrent > 0 ? 100.0 : 0.0);
                ...
                <table class="min-w-full text-sm text-left text-gray-700 dark:text-gray-200">
                    <thead
                        class="sticky top-0 z-10 text-xs uppercase bg-gray-50/80 backdrop-blur supports-backdrop-blur:backdrop-blur-sm dark:bg-gray-800/80 dark:text-gray-300">
                        <tr class="divide-x divide-gray-200 dark:divide-gray-700">
                            <th class="px-4 py-3 font-semibold">Product</th>
                            @foreach ($monthlyReport['months'] ?? [] as $m)
                                <th class="px-3 py-3 font-semibold text-right whitespace-nowrap">{{ $m['label'] }}
                                </th>
                            @endforeach
                        ...
                        <tr class="divide-x divide-gray-200 dark:divide-gray-700 bg-white/60 dark:bg-gray-900/40">
                                    ...
                                    $cellClass =
                                        $mk === $totalMaxKey
                                            ...
                                            : ($mk === $totalMinKey
                                                ...
                                                : '');
                                @endphp
                                <th class="px-3 py-2 text-right tabular-nums {{ $cellClass }}">
                                    {{ number_format($amt, $metricDecimals) }}</th>
                            @endforeach
                            <th class="px-3 py-2 tabular-nums">{{ number_format($totalFocus, $metricDecimals) }}</th>
                            <th class="px-3 py-2 whitespace-nowrap">
                                <span
                                    class="font-medium">{{ $monthLabelByKey[$totalMaxKey] ?? ($totalMaxKey ?? '-') }}</span>
                                <span
                                    class="text-gray-500 dark:text-gray-400">({{ number_format((float) ($totalMaxAmount ?? 0), $metricDecimals) }})</span>
                            </th>
                            <th class="px-3 py-2 whitespace-nowrap">
                                <span
                                    class="font-medium">{{ $monthLabelByKey[$totalMinKey] ?? ($totalMinKey ?? '-') }}</span>
                                <span
                                    class="text-gray-500 dark:text-gray-400">({{ number_format((float) ($totalMinAmount ?? 0), $metricDecimals) }})</span>
                            </th>
                            <th class="px-3 py-2 whitespace-nowrap">
                                <div class="font-semibold tabular-nums text-gray-900 dark:text-white">
                                    {{ number_format($totalCurrent, $metricDecimals) }}</div>
                            ...
                            @php
                                ...
                                $totalFocusTrendText =
                                    $totalFocusDiff > 0 ? 'Above' : ($totalFocusDiff < 0 ? 'Below' : 'On');
                                $totalFocusTrendClass =
                                    $totalFocusDiff > 0
                                        ? 'text-emerald-700 dark:text-emerald-300'
                                        : ($totalFocusDiff < 0
                                            ? 'text-red-700 dark:text-red-300'
                            ...
                            <th class="px-3 py-2 whitespace-nowrap">
                                ...
                                <div class="text-xs {{ $totalFocusTrendClass }}">{{ $totalFocusTrendText }} focus
                                </div>
                            </th>
                        </tr>
                    </thead>
                        ...
                        @forelse (($monthlyReport['rows'] ?? []) as $row)
                            @php
                                ...
                                foreach ($monthlyReport['months'] ?? [] as $m) {
                                    ...
                                    if ($rowMaxAmount === null || $amt > $rowMaxAmount) {
                                        $rowMaxAmount = $amt;
                                        $rowMaxKey = $mk;
                                    }
                                    if ($rowMinAmount === null || $amt < $rowMinAmount) {
                                        $rowMinAmount = $amt;
                                        $rowMinKey = $mk;
                                    }
                                }
                                ...
                                $current = $currentKey
                                    ? $calcMetric((float) ($row['sales'][$currentKey] ?? 0), $weight)
                                    : 0.0;
                            ...
                            <tr class="bg-white dark:bg-gray-900 hover:bg-gray-50 dark:hover:bg-gray-800/60">
                                ...
                                <td class="px-3 py-3 whitespace-nowrap">
                                    ...
                                    <span
                                        class="text-gray-500 dark:text-gray-400">({{ number_format((float) ($rowMaxAmount ?? 0), $metricDecimals) }})</span>
                                </td>
                                <td class="px-3 py-3 whitespace-nowrap">
                                    <span class="font-medium">
                                    ...
                                    <span
                                        class="text-gray-500 dark:text-gray-400">({{ number_format((float) ($rowMinAmount ?? 0), $metricDecimals) }})</span>
                                </td>
                                <td class="px-3 py-3 whitespace-nowrap">
                                    <div class="font-semibold tabular-nums text-gray-900 dark:text-white">
                                        {{ number_format($current, $metricDecimals) }}</div>
        ...
        <div class="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-lg overflow-hidden">
            <div class="p-4">
                <div class="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
                    <div>
                        ...
                        <div class="text-sm text-gray-500 dark:text-gray-300">Compare actual sale totals by product
                        </div>
                    </div>

                    <div class="flex items-center gap-2">
                        <x-button sm teal flat icon="information-circle" label="အသုံးပြုပုံ"
                    ...
                    <div class="grid gap-3 sm:grid-cols-3 lg:grid-cols-5">
                        <div class="sm:col-span-1">
                            <label
                                class="block text-sm font-medium text-gray-700 dark:text-gray-200 mb-1">Branch</label>
                            <select wire:model.live="monthly_branch_id"
                        ...
                        <div class="sm:col-span-2" wire:ignore>
                            <div x-data="{
                                ...
                                <label class="block text-sm font-medium text-gray-700 dark:text-gray-200 mb-1">Range
                                    A</label>
                        ...
                        <div class="sm:col-span-1">
                            <label
                                class="block text-sm font-medium text-gray-700 dark:text-gray-200 mb-1">Compare</label>
                        ...
                        <div class="sm:col-span-1">
                            <label
                                class="block text-sm font-medium text-gray-700 dark:text-gray-200 mb-1">Metric</label>
                            <select wire:model.live="report_metric"
                                class="block w-full border rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-800 dark:text-white">
                                <option value="qty">Quantity</option>
                                <option value="grams">Total grams</option>
                        ...
                        <div class="sm:col-span-2" wire:ignore>
                            ...
                            }" x-init="$nextTick(() => initPicker())" x-show="mode === 'custom'"
                                style="display:none">
                                <label class="block text-sm font-medium text-gray-700 dark:text-gray-200 mb-1">Range
                                    B</label>
                            ...
                            <div x-show="@entangle('sale_compare_mode').live === 'last_month'"
                                class="text-xs text-gray-500 dark:text-gray-400 pt-6" style="display:none">
                                Range B auto = last month (same days)
                            </div>

                            <div x-show="@entangle('sale_compare_mode').live === 'prev'"
                                class="text-xs text-gray-500 dark:text-gray-400 pt-6" style="display:none">
                ...
                <div class="mt-3 text-xs text-gray-500 dark:text-gray-400">
                    Tip: Default Range A = last 7 days. Default compare = last month (same days). Focus A/B = focus
                    per-day × days.
                </div>
            </div>
            ...
            @if ($rangeCompare)
                <div class="px-4 pb-4 text-sm text-gray-700 dark:text-gray-200">
                    <span class="font-semibold">Range A:</span> {{ $rangeCompare['rangeA']['from'] }} →
                    {{ $rangeCompare['rangeA']['to'] }}
                    <span class="mx-2 text-gray-300">|</span>
                    <span class="font-semibold">Range B:</span> {{ $rangeCompare['rangeB']['from'] }} →
                    {{ $rangeCompare['rangeB']['to'] }}
                </div>
                ...
                <div class="overflow-auto max-h-[60vh]">
                    @php
                        ...
                        $daysA = $rangeDays(
                            $rangeCompare['rangeA']['from'] ?? null,
                            $rangeCompare['rangeA']['to'] ?? null,
                        );
                        $daysB = $rangeDays(
                            $rangeCompare['rangeB']['from'] ?? null,
                            $rangeCompare['rangeB']['to'] ?? null,
                        );
                    ...
                    <table class="min-w-full text-sm text-left text-gray-700 dark:text-gray-200">
                        <thead
                            ...
                            <tr class="divide-x divide-gray-200 dark:divide-gray-700">
                                <th class="px-4 py-3 font-semibold">Product</th>
                                <th class="px-3 py-3 font-semibold whitespace-nowrap">Focus A ({{ $metricLabel }})
                                </th>
                                ...
                                <th class="px-3 py-3 font-semibold whitespace-nowrap">Focus B ({{ $metricLabel }})
                                </th>
                            ...
                            <tr class="divide-x divide-gray-200 dark:divide-gray-700 bg-white/60 dark:bg-gray-900/40">
                                <th class="px-4 py-2 font-semibold text-gray-900 dark:text-white">Grand Total</th>
                                <th class="px-3 py-2 tabular-nums">{{ number_format($totalFocusA, $metricDecimals) }}
                                </th>
                                <th class="px-3 py-2 text-right tabular-nums">
                                    {{ number_format($totalA, $metricDecimals) }}</th>
                                @php
                                    $totalAFocusDiff = (float) $totalA - (float) $totalFocusA;
                                    $totalAFocusTrendText =
                                        $totalAFocusDiff > 0 ? 'Above' : ($totalAFocusDiff < 0 ? 'Below' : 'On');
                                    $totalAFocusTrendClass =
                                        $totalAFocusDiff > 0
                                            ? 'text-emerald-700 dark:text-emerald-300'
                                            : ($totalAFocusDiff < 0
                                                ? 'text-red-700 dark:text-red-300'
                                ...
                                <th class="px-3 py-2 tabular-nums">{{ number_format($totalFocusB, $metricDecimals) }}
                                </th>
                                <th class="px-3 py-2 text-right tabular-nums">
                                    {{ number_format($totalB, $metricDecimals) }}</th>
                                ...
                                <th class="px-3 py-2 whitespace-nowrap">
                                    ...
                                    <span
                                        class="text-xs text-gray-500 dark:text-gray-400">({{ number_format($totalPct, 1) }}%)</span>
                                </th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                            @foreach ($rangeCompare['rows'] as $row)
                                ...
                                <tr class="bg-white dark:bg-gray-900 hover:bg-gray-50 dark:hover:bg-gray-800/60">
                                    <td
                                        class="px-4 py-3 font-semibold text-gray-900 dark:text-white whitespace-nowrap">
                                        {{ $row['product'] ?? '-' }}
                                    </td>
                                    <td class="px-3 py-3 whitespace-nowrap tabular-nums">
                                        @php
    ...
    <div class="my-6">
        <div class="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-lg overflow-hidden">
            <div class="p-4">
                <div class="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
                    <div>
                        <div class="text-lg font-semibold text-gray-900 dark:text-white">Focus vs Actual (Line Chart)
                        </div>
                        <div class="text-sm text-gray-500 dark:text-gray-300">Custom date range (default: current
                            month)</div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-3 gap-3 items-end">
                        ...
                        <div wire:ignore>
                            <div x-data="{
                                ...
                                <label class="block text-sm font-medium text-gray-700 dark:text-gray-200 mb-1">Date
                                    Range</label>
            ...
            <div class="px-4 pb-4" wire:ignore>
                <div x-data="{
                    ...
                    <div class="flex items-center justify-between mb-2">
                        ...
                        <div x-show="loading" class="text-xs text-gray-500 dark:text-gray-400" style="display:none">
                            Loading…</div>
                    </div>
                    <div x-ref="chart"></div>
                </div>
    ...
    <div class="my-6">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 items-end">
                            ...
                            <div wire:ignore>
                                <div x-data="{
                                    ...
                                    <label
                                        class="block text-sm font-medium text-gray-700 dark:text-gray-200 mb-1">Month</label>
                                    <input x-ref="month" type="text" class="w-full"
                                        placeholder="Select month" />
                                </div>
                            </div>

                            <div class="flex justify-end">
                                <x-button sm positive icon="download" label="Export Excel"
                                    wire:click="exportStockoutMonthlyReport" wire:loading.attr="disabled" />
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <table class="min-w-full text-sm text-left text-gray-700 dark:text-gray-200">
                        <thead
                            ...
                            <tr class="divide-x divide-gray-200 dark:divide-gray-700 bg-white/60 dark:bg-gray-900/40">
                                ...
                                <th class="px-3 py-2 text-right tabular-nums">
                                    {{ number_format((float) ($stockoutTotals['opening'] ?? 0), 0) }}</th>
                                <th class="px-3 py-2 text-right tabular-nums">
                                    {{ number_format((float) ($stockoutTotals['refill'] ?? 0), 0) }}</th>
                                <th class="px-3 py-2 text-right tabular-nums">
                                    {{ number_format((float) ($stockoutTotals['sale'] ?? 0), 0) }}</th>
                                <th class="px-3 py-2 text-right tabular-nums">
                                    {{ number_format((float) ($stockoutTotals['closing'] ?? 0), 0) }}</th>
                                <th class="px-3 py-2 text-right tabular-nums">
                                    {{ (int) ($stockoutTotals['zero_days'] ?? 0) }}</th>
                                <th class="px-3 py-2 text-right tabular-nums">
                                    {{ number_format((float) ($stockoutTotals['loss_index'] ?? 0), 2) }}</th>
                                <th class="px-4 py-2">-</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                            @forelse (($stockoutReport['rows'] ?? []) as $r)
                                <tr class="bg-white dark:bg-gray-900 hover:bg-gray-50 dark:hover:bg-gray-800/60">
                                    <td
                                        class="px-4 py-3 font-semibold text-gray-900 dark:text-white whitespace-nowrap">
                                        {{ $r['product'] ?? '-' }}</td>
                                    <td class="px-4 py-3 text-gray-700 dark:text-gray-200">{{ $r['remark'] ?? '' }}
                                    </td>
                                    <td class="px-3 py-3 text-right tabular-nums">
                                        {{ number_format((float) ($r['opening'] ?? 0), 0) }}</td>
                                    <td class="px-3 py-3 text-right tabular-nums">
                                        {{ number_format((float) ($r['refill'] ?? 0), 0) }}</td>
                                    <td class="px-3 py-3 text-right tabular-nums">
                                        {{ number_format((float) ($r['sale'] ?? 0), 0) }}</td>
                                    <td class="px-3 py-3 text-right tabular-nums">
                                        {{ number_format((float) ($r['closing'] ?? 0), 0) }}</td>
                                    <td class="px-3 py-3 text-right tabular-nums">
                                        <span
                                            class="font-semibold {{ ((int) ($r['zero_days'] ?? 0)) > 0 ? 'text-red-700 dark:text-red-300' : 'text-gray-700 dark:text-gray-200' }}">
                                            {{ (int) ($r['zero_days'] ?? 0) }}
                                        </span>
                                    </td>
                                    <td class="px-3 py-3 text-right tabular-nums">
                                        <span
                                            class="font-semibold {{ ((float) ($r['loss_index'] ?? 0)) > 0 ? 'text-red-700 dark:text-red-300' : 'text-gray-700 dark:text-gray-200' }}">
        ...
        <x-modal.card title="Range Report Manual (အသုံးပြုပုံ)" blur wire:model="rangeManualModal">
            <div class="space-y-3 text-sm text-gray-700 dark:text-gray-200">
                ...
                <div>
                    ...
                    <div>
                        <span class="font-semibold">Focus A</span> / <span class="font-semibold">Focus B</span> =
                        Focus qty (per-day) × Range days ကို Metric အလိုက်တွက်ပြထားတာပါ
                    </div>
                    <div>
                        <span class="font-semibold">A vs Focus</span> = Range A − Focus A
                    </div>
                    <div>
                        <span class="font-semibold">Δ</span> = Range A − Range B ၊ <span
                            class="font-semibold">Situation</span> = Up/Down/Flat + %
                    </div>
                </div>
            </div>
                    ...
                    <tbody>
                        @foreach ($products as $product)
                            <tr
                                ...
                                @foreach ($branches as $branch)
                                    <td
                                        class="px-4 py-3 font-semibold text-center text-gray-900 md:px-6 dark:text-white">
        ...
        <x-modal.card title="{{ $productSummary['detail'] }}" wire:model='productSummaryModal'>
            ...
            <x-button red icon="pencil" label="Edit Product"
                href="{{ route('edit_product', ['selected' => $productIdFilter]) }}" />
            ...
            <div class="container mx-auto my-2 overflow-x-auto">
                <div style="display: none" class="grid grid-cols-2 gap-2 my-2 bg-white dark:bg-gray-900">
                    <x-select wire:model.live="hashtag_id" placeholder="#hash-tag" :async-data="route('hashtag')"
                ...
                <table
                    class="w-full text-sm text-left text-gray-500 rtl:text-right dark:text-gray-400 overflow-x-auto">
                    <thead>
                    ...
                    <tbody>
                        @forelse ($productSummary['branches'] as $data)
                            <tr
                                ...
                                <td class="px-6 py-4" title="Latest focus quantity">
                                    {{ $data['latest_focus_qty'] ?? 0 }}</td>
                                <td class="px-6 py-4" title="Average daily sales">
                                    {{ number_format($data['avg_sales']) }}</td>
                                <td class="px-6 py-4" title="Current inventory balance">
                                    {{ number_format($data['balance']) }}</td>
                                <td class="px-6 py-4 {{ $data['remaining_days'] !== null && $data['remaining_days'] < 7 ? 'text-red-500 font-bold' : '' }}"
            ...
            <x-card title="Choose a function">
                ...
                <ol class="">
                    <li class="hover:text-gray-500 hover:underline">
                        <a href="{{ route('focus', ['brch' => $branchId, 'prod' => $productId]) }}"
                            wire:navigate>Product
                            Focus</a>
                    </li>
                    <li class="hover:text-gray-500 hover:underline">
                        <a href="{{ route('focus', ['brch' => $branchId, 'prod' => $productId]) }}"
