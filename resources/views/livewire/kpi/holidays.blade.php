<div class="space-y-6">
    <section class="rounded-3xl bg-slate-900 px-6 py-7 text-white">
        ...
        <p class="mt-3 max-w-3xl text-sm text-slate-200">
            Holidays exclude one employee from daily KPI tasks for the affected day and gray out that date in the audit
            matrix.
        </p>
    </section>
    ...
    @if (session()->has('message'))
        <div
            class="rounded-2xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm text-emerald-700 dark:border-emerald-900 dark:bg-emerald-950/40 dark:text-emerald-300">
            {{ session('message') }}
        </div>
    @endif
    ...
    <article class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-700 dark:bg-slate-900">
        <div class="flex items-center justify-between">
            ...
            @if ($editingHolidayId)
                <button type="button" wire:click="cancelHoliday"
        ...
        <form wire:submit="{{ $editingHolidayId ? 'updateHoliday' : 'createHoliday' }}" class="mt-4 space-y-4">
            <div class="grid gap-4 md:grid-cols-2">
                <div>
                    ...
                    <input type="date" wire:model.defer="holidayDate"
                ...
                <div>
                    ...
                    <div class="mt-1">
                        <x-select label="" placeholder="Search employee" wire:model="holidayUserId"
            ...
            <div>
                ...
                <input type="text" wire:model.defer="holidayName"
                    class="mt-1 w-full rounded-xl border-slate-300 text-sm shadow-sm focus:border-slate-500 focus:ring-slate-500 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-100"
            ...
            <div>
                ...
                <textarea wire:model.defer="holidayRemark" rows="3"
                    class="mt-1 w-full rounded-xl border-slate-300 text-sm shadow-sm focus:border-slate-500 focus:ring-slate-500 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-100"
                    placeholder="Optional note for this holiday"></textarea>
                @error('holidayRemark')
                    <p class="mt-1 text-xs text-rose-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <label class="flex items-center gap-2 text-sm text-slate-700 dark:text-slate-200">
                <input type="checkbox" wire:model.defer="holidayIsActive"
                    ...
                    <div class="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                        <div>
                            <div class="flex flex-wrap items-center gap-2">
                                ...
                                <span
                                    class="rounded-full px-2 py-0.5 text-xs {{ $holiday->is_active ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/40 dark:text-emerald-300' : 'bg-slate-200 text-slate-600 dark:bg-slate-700 dark:text-slate-300' }}">
                                    {{ $holiday->is_active ? 'Active' : 'Inactive' }}
                                </span>
                            </div>
                            <p class="mt-1 text-sm text-slate-600 dark:text-slate-300">
                                {{ $holiday->holiday_date?->format('Y-m-d') }} -
                                {{ $holiday->user?->name ?? 'Unknown employee' }}
                            </p>
                        ...
                        <div class="flex gap-2">
                            <button type="button" wire:click="editHoliday({{ $holiday->id }})"
                            ...
                            <button type="button" wire:click="deleteHoliday({{ $holiday->id }})"
