                    ...
                    <div x-show="steps.length > 0 && steps[carouselIndex]" class="space-y-6">
                        ...
                        <div class="rounded-2xl border border-slate-100 bg-slate-50/50 p-5 dark:border-slate-800 dark:bg-slate-800/40">
                                ...
                                <div class="space-y-2">
                                    <div class="flex flex-wrap items-center gap-2">
                                        <h4 class="text-lg font-semibold text-slate-900 dark:text-slate-100" 
                                            x-text="steps[carouselIndex]?.submission?.instance?.template?.title">
                                        </h4>
                                        <span class="rounded-full bg-sky-100 px-2.5 py-0.5 text-xs font-medium uppercase tracking-[0.15em] text-sky-700 dark:bg-sky-950 dark:text-sky-300"
                                              x-text="'Step ' + steps[carouselIndex]?.step_order">
                                        </span>
                                        <span class="rounded-full bg-slate-100 px-2.5 py-0.5 text-xs font-medium uppercase tracking-[0.15em] text-slate-700 dark:bg-slate-800 dark:text-slate-300"
                                              x-text="steps[carouselIndex]?.submission?.instance?.template?.group?.name || 'No KPI Group'">
                                        </span>
                                        <span class="rounded-full bg-slate-100 px-2.5 py-0.5 text-xs font-medium uppercase tracking-[0.15em] text-slate-700 dark:bg-slate-800 dark:text-slate-300"
                                              x-text="steps[carouselIndex]?.submission?.instance?.template?.frequency || ''">
                                        </span>
                                    </div>
                                    <p class="text-sm font-medium text-slate-700 dark:text-slate-300">
                                        Employee: <span class="text-slate-900 dark:text-white font-semibold" x-text="steps[carouselIndex]?.submission?.instance?.user?.name || '-'"></span>
                                    </p>
                                    <div class="grid gap-2 text-sm text-slate-500 dark:text-slate-400 md:grid-cols-2">
                                        <p>Submitted: <span x-text="steps[carouselIndex]?.submission?.submitted_at ? new Date(steps[carouselIndex].submission.submitted_at).toLocaleString() : '-'"></span></p>
                                        <p>Due: <span x-text="steps[carouselIndex]?.submission?.instance?.due_at ? new Date(steps[carouselIndex].submission.instance.due_at).toLocaleString() : 'No cutoff'"></span></p>
                                        <p>On Time: <span :class="steps[carouselIndex]?.submission?.is_late ? 'text-rose-600 font-semibold' : 'text-emerald-600 font-semibold'" x-text="steps[carouselIndex]?.submission?.is_late ? 'Late' : 'On time'"></span></p>
                                        <p>Submitted By: <span x-text="steps[carouselIndex]?.submission?.submitted_by?.name || steps[carouselIndex]?.submission?.submittedBy?.name || '-'"></span></p>
                                    </div>
                                </div>
                                
                                <!-- Action Info Badge -->
                                <div class="text-right text-xs text-slate-400 dark:text-slate-500" x-text="'Card ' + (carouselIndex + 1) + ' of ' + steps.length">
                                </div>
                            </div>
                            ...
                            <div class="mt-4">
                                ...
                                <div class="flex flex-wrap gap-2">
                                    <template x-for="apStep in (steps[carouselIndex]?.submission?.approval_steps || steps[carouselIndex]?.submission?.approvalSteps || []).slice().sort((a,b) => a.step_order - b.step_order)" :key="apStep.id">
                                        <span class="inline-flex items-center gap-1.5 rounded-lg px-2 py-1 text-xs font-medium border"
                                              :class="{
                                                  'bg-emerald-50 text-emerald-700 border-emerald-200 dark:bg-emerald-950/30 dark:text-emerald-400 dark:border-emerald-900/50': apStep.status === 'approved',
                                                  'bg-amber-50 text-amber-700 border-amber-200 dark:bg-amber-950/30 dark:text-amber-400 dark:border-amber-900/50': apStep.status === 'pending',
                                                  'bg-rose-50 text-rose-700 border-rose-200 dark:bg-rose-950/30 dark:text-rose-400 dark:border-rose-900/50': apStep.status === 'rejected',
                                                  'bg-slate-50 text-slate-600 border-slate-200 dark:bg-slate-800/80 dark:text-slate-400 dark:border-slate-700': apStep.status === 'cancelled'
                                              }"
                                              x-text="'Step ' + apStep.step_order + ': ' + (apStep.approver?.name || 'Unassigned') + ' (' + apStep.status.replace('_', ' ') + ')'">
                                        </span>
                                    </template>
                                </div>
                            </div>
                        ...
                        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between pt-2">
                            ...
                            <div class="flex items-center gap-2">
                                ...
                                <span class="text-sm font-medium text-slate-600 dark:text-slate-400" x-text="(carouselIndex + 1) + ' / ' + steps.length">
                                </span>
                                
                                <button type="button" @click="carouselIndex = Math.min(steps.length - 1, carouselIndex + 1)" :disabled="carouselIndex === steps.length - 1"
                                        class="inline-flex items-center justify-center h-10 w-10 rounded-xl border border-slate-300 bg-white text-slate-600 hover:bg-slate-50 disabled:opacity-40 disabled:hover:bg-white dark:border-slate-700 dark:bg-slate-800 dark:text-slate-300 dark:hover:bg-slate-700">
                                    <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                            ...
                            <div class="flex items-center gap-3">
                                <!-- Reject -->
                                <button type="button" 
                                        @click="if (remark.trim() === '') { alert('Remark is required to reject!'); return; } $wire.rejectStepDirectly(steps[carouselIndex].id, remark); remark = '';"
                                ...
                                <!-- Approve -->
                                <button type="button" 
                                        @click="$wire.approveStepDirectly(steps[carouselIndex].id, remark); remark = '';"
                                ...
                                <!-- Next Slide (Skip) -->
                                <button type="button" x-show="carouselIndex < steps.length - 1" @click="carouselIndex = carouselIndex + 1"
