<div>
                    ...
                    <tr>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="text-sm font-medium text-gray-900 dark:text-gray-100">
                                {{ $asset->item->name ?? $asset->name }}</div>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">
                            @if ($asset->batch)
                                <div class="flex items-center gap-2">
                                    ...
                                    <x-button.circle xs icon="pencil" wire:click="editBatch({{ $asset->batch->id }})"
                                        title="Edit Batch" />
                                </div>
                                <div class="text-xs">
                                ...
                                <div class="text-xs">
                                    Cost range: {{ number_format($asset->batch->minimum_cost, 2) }} -
                                    {{ number_format($asset->batch->maximum_cost, 2) }}
                                </div>
                                <div class="text-xs">
                                ...
                                <div class="mt-1">
                                    <span
                                        class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full {{ $batchStatus === 'high' ? 'bg-red-100 text-red-800' : ($batchStatus === 'low' ? 'bg-yellow-100 text-yellow-800' : 'bg-green-100 text-green-800') }}">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            <span
                                class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full {{ $asset->balance <= $asset->minimum_balance ? 'bg-red-100 text-red-800' : 'bg-green-100 text-green-800' }}">
                                {{ $asset->balance }}
                            </span>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium space-x-2">
                            <x-button.circle positive icon="plus"
                                wire:click="createTransaction({{ $asset->id }}, 'in')" title="Asset In" />
                            <x-button.circle negative icon="minus"
                                wire:click="createTransaction({{ $asset->id }}, 'out')" title="Asset Out" />
        ...
        <x-card title="{{ $itemId ? 'Edit Item' : 'Create Item' }}">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div class="flex gap-2 items-end">
                    ...
                    <div class="pb-1">
                        <x-button flat label="New Category" wire:click="createCategory" />
                    </div>
                </div>
    ...
    <!-- Category Modal -->
    <x-modal wire:model.defer="showCategoryModal">
        <x-card title="{{ $categoryId ?? null ? 'Edit Category' : 'Create Category' }}">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <x-input label="Category Name" wire:model.defer="category_name" />
                <div class="md:col-span-2">
                    <x-textarea label="Description" wire:model.defer="category_description" />
                </div>
            </div>
            <x-slot name="footer">
                <div class="flex justify-end gap-x-4">
                    <x-button flat label="Cancel" x-on:click="close" />
                    <x-button primary label="Save" wire:click="saveCategory" />
                </div>
            </x-slot>
        </x-card>
    </x-modal>
