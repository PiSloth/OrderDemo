                    ...
                    <div class="min-h-0 flex-1 overflow-y-auto">
                        @forelse ($boardContents as $content)
                            <button type="button" wire:key="whiteboard-item-{{ $content->id }}"
                                ...
                                @click="closeTooltip(); if (window.innerWidth < 1024) { listOpen = false; }"
                                        ...
                                        <div class="ml-auto flex items-center gap-1.5 overflow-visible">
                                            ...
                                            @if ($this->canManageSelectedContent())
                                                @if (!$selectedContent->trashed())
                                                    <div class="relative">
                                                        <button type="button"
                                                            wire:click="startEditingSelectedContent"
                                                            @mouseenter="showTooltip('content-edit')"
                                                            @mouseleave="closeTooltip()"
                                                            @focus="showTooltip('content-edit')"
                                                            @blur="closeTooltip()"
                                                ...
                                                <div class="relative">
                                                    <button type="button"
                                                        @mouseenter="showTooltip('content-archive')"
                                                        @mouseleave="closeTooltip()"
                            ...
                            <div class="space-y-6">
                                ...
                                <section class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                                    <div class="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                                        <div>
                                            ...
                                            <p class="text-sm text-slate-500">Add the decision as rich text. If your
                                                Google Calendar is connected, your current calendar events are shown
                                                here.</p>
                                        </div>
                                    ...
                                    <form wire:submit.prevent="submitDecision" class="mt-5 space-y-4">
                                        ...
                                        @if ($googleCalendarConnected)
                                            <div class="rounded-2xl border border-sky-100 bg-sky-50 p-4">
                                                <div class="mb-3 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                                                    <div>
                                                        <p class="text-sm font-semibold text-sky-900">Google Calendar Events</p>
                                                        <p class="text-xs text-sky-700">
                                                            Connected{{ $googleCalendarEmail ? ' as ' . $googleCalendarEmail : '' }}
                                                        </p>
                                                    </div>
                                                    <a href="{{ route('calendar.index') }}"
                                                        class="inline-flex items-center rounded-lg border border-sky-200 bg-white px-3 py-2 text-xs font-medium text-sky-700 hover:bg-sky-100">
                                                        Open Full Calendar
                                                    </a>
                                                </div>

                                                <div wire:ignore class="min-h-[26rem] rounded-2xl bg-white p-2"
                                        ...
                                        @else
                                            <div class="rounded-2xl border border-dashed border-slate-300 bg-slate-50 p-4">
                                                <p class="text-sm font-medium text-slate-700">Google Calendar is not connected.</p>
                                                <p class="mt-1 text-xs text-slate-500">Connect it to view existing calendar events while writing the decision.</p>
                                                <a href="{{ route('calendar.socialite.connect', ['redirect_to' => 'whiteboard.board']) }}"
                                                    class="mt-3 inline-flex items-center rounded-lg bg-sky-600 px-3 py-2 text-xs font-medium text-white hover:bg-sky-700">
                                ...
                                <section class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                                    <div class="flex items-center justify-between gap-3">
                                        ...
                                        <span
                                            class="text-sm text-slate-500">{{ $selectedContent->decisions->count() }}
                                            entries</span>
                                    </div>
    ...
    <script>
        document.addEventListener('livewire:init', () => {
            ...
            Livewire.on('whiteboard-decision-reset', () => {
                ...
                window.setTimeout(() => {
                    if (typeof window.bootGoogleCalendars === 'function') {
                        window.bootGoogleCalendars(document);
                    }
                }, 0);
            });
